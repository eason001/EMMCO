...
public class EGT extends Algorithm {
	...
	private SolutionSet populationFinal;
	private SolutionSet auxpopulationFinal;
	...
	public SolutionSet execute() throws JMException, ClassNotFoundException {
		...
		// Create the initial solutionSet DYNAMIC
	//	if(EGT_main.getTermination()==EGT_main.getTimes()){ // != normal
		if(EGT_main.getCtimes()!=0){
			auxpopulationFinal = EGT_main.getpopulationFinal();
			//System.out.println("should be 20 and it is: " + auxpopulationFinal.size());
			Solution newSolution;
			Solution domSolution;
			for (int j = 0; j < maxPopulations; j++) {
				populationList[j]= new SolutionSet(populationSize);
				/*if(auxpopulationFinal.size()>j){				
				  domSolution = auxpopulationFinal.get(j);
				}*/
				Pop_Index = j;
				for (int i = 0; i < populationSize; i++) {
					if(i<(populationSize/2)){//dom rate populationSize/2 0
						if(auxpopulationFinal.size()>j){		
							  domSolution = auxpopulationFinal.get(j);
							  switch(EGT_main.LSoption){						 
								  case 0:
								//	  domSolution = localSearch_mutation(domSolution);
									  mutationOperator.execute(domSolution);
									  break;
								  case 1:
									  domSolution = localSearch_tabu(domSolution);
									  break;
								  case 2:
									  domSolution = localSearch_tabuR(domSolution);
									  break;
								  default:
									  domSolution = localSearch_mutation(domSolution);
									  break;
							  }
							  populationList[j].add(domSolution);
							}else{
...
/*
				...
				//	VMDeployment.updateHOST(solutionaux); //UPDATE HOST
					...
					HV+=HVlist[j];
					BWCon+=solutionaux.getObjective(1);
					BodyCloudOptimization.setMax_BW(BodyCloudOptimization.getMax_BW()-solutionaux.getObjective(1));
			} //end for all population 	  
		//	long estimatedTime = System.currentTimeMillis() - initTime;
		//    System.out.println("execution time: "+estimatedTime + "ms");
				  EGT_main.setpopulationFinal(populationFinal);
		    	  HV = HV/maxPopulations;
		    	  EGT_main.hv[runs][evaluations]=HV;
		    	  System.out.println(evaluations + ": " + (double)HV);
		    	  //Termination condition
		     	  if (Math.abs(HV-diff)>EGT_main.getDiff()){
		    		 counter = 0;
		    		 diff=HV;
		    	  }else{
		    		  counter++;
		    	  ...
		    	  if(counter>=EGT_main.getTermination()){
		    		  stop = true;
		    	  }
	...
	public class CustomComparator implements Comparator<Solution> {
	  //  @Override
	    ...
	    public int compare(Solution o1, Solution o2) {
	    	...
	    	So1.add(o1);
	    	So2.add(o2);
	        return (int)indicators.getHypervolume(So1)-(int)indicators.getHypervolume(So2);
	    }
	}
	...
	private Solution startBattle(Solution p1, Solution p2){
		...
		/*
		if( op == 5 ) winner = compareTwoObj(p1, p2, 0, 1 );
