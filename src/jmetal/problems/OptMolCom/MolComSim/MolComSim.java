...
public class MolComSim {
	...
	public int runSim(Map<String,String> args) throws IOException {
		//MolComSim molComSim = createInstance();
		return run2(args);
	}
	
	private int run2(Map<String,String> args)  throws IOException {
		startSim2(args);
	...
	public void startSim2(Map<String,String> args) throws IOException {
		...
		createMedium();
		createMicrotubules(); 
		createNanoMachines();	//CREATE TX RX INTERMEDIANNODE
	...
	private void createNanoMachines() {
		...
		for (IntermediateNodeParam inp : simParams.getIntermediateNodeParams()) {
			NanoMachine nm = NanoMachine.createIntermediateNode(inp.getCenter(), inp.getRadius(), 
					inp.getInfoMolReleasePoint(), inp.getAckMolReleasePoint(), inp.getInfoParams(), inp.getAckParams(), this); // CHANGE HERE INFOPARAMS and ACKPARAMS TO DVs like number of molecules, active passive, info ack
			growNanoMachine(nm); // adds NanoMachine to medium's grid
			transmitters.add(nm);
			receivers.add(nm);
			nanoMachines.add(nm);			
