...
public class SimulationParams {
	...
	public SimulationParams(Map<String,String> args) {
		setInitialMovementDefaults();
		try {
			readParamsFile2(args);
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	private void readParamsFile2(Map<String,String> map) throws IOException{
		
		mediumLength = Integer.parseInt(map.get("mediumDimensionX"));
		mediumWidth = Integer.parseInt(map.get("mediumDimensionY"));
		mediumHeight = Integer.parseInt(map.get("mediumDimensionZ"));
		
		molRandMoveX = Integer.parseInt(map.get("stepLengthX"));
		molRandMoveY = Integer.parseInt(map.get("stepLengthY"));
		molRandMoveZ = Integer.parseInt(map.get("stepLengthZ"));
		...
		useAcknowledgements = (Integer.parseInt(map.get("useAcknowledgements")) == 1) ? true : false;
		decomposing = (Integer.parseInt(map.get("decomposing")) == 1) ? true : false;
		
		velRail = Integer.parseInt(map.get("velRail"));
		probDRail = Double.parseDouble(map.get("probDRail"));
	
		transmitterParams.add(new NanoMachineParam(new Scanner(map.get("transmitter"))));
		receiverParams.add(new NanoMachineParam(new Scanner(map.get("receiver"))));
		intermediateNodeParams.add(new IntermediateNodeParam(new Scanner(map.get("intermediateNodeA")))); 
		intermediateNodeParams.add(new IntermediateNodeParam(new Scanner(map.get("intermediateNodeB")))); 
		intermediateNodeParams.add(new IntermediateNodeParam(new Scanner(map.get("intermediateNodeC")))); 
		intermediateNodeParams.add(new IntermediateNodeParam(new Scanner(map.get("intermediateNodeD")))); 
		intermediateNodeParams.add(new IntermediateNodeParam(new Scanner(map.get("intermediateNodeE")))); 
		moleculeParams.add(new MoleculeParams(new Scanner(map.get("moleculeParamsINFO"))));
		moleculeParams.add(new MoleculeParams(new Scanner(map.get("moleculeParamsACK"))));
		moleculeParams.add(new MoleculeParams(new Scanner(map.get("moleculeParamsNOISE"))));
		microtubuleParams.add(new MicrotubuleParams(new Scanner(map.get("microtubuleParams"))));
		
		outputFileName = map.get("outputFile");
	...
	private void readParamsFile(String fName) throws IOException{
		...
		while((line = br.readLine())!=null){
			...
			if(line.startsWith("stepLengthX")){
				...
				System.out.println("parameters: " + line.substring(line.indexOf(" ")));
				moleculeParams.add(
						new MoleculeParams(
								new Scanner(
										line.substring(line.indexOf(" ")))));
